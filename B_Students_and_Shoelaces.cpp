...
ll solve()
{
        ...
        while(1) {
            ...
            for(int i=1; i<=n; i++) {
                if(st[i].size()==1 && !mark[min((ll)i,*(st[i].begin()))][max((ll)i,*(st[i].begin()))]) {
                    vp.push_back({i,*(st[i].begin())});
                    mark[min((ll)i,*(st[i].begin()))][max((ll)i,*(st[i].begin()))] = 1;
                }
            }
            if(!vp.size()) break;
            for(auto x:vp) {
                st[x.first].erase(st[x.first].find(x.second));
