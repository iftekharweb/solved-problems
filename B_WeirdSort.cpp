...
void DFS(ll n , vector<ll> &a, vector<ll> &vis, vector<vector<ll> > &adj, vector<ll> &dhuka) {
        vis[n] = 1;
        dhuka.push_back(a[n]);
        for(auto x:adj[n]) {
            if(!vis[x]) DFS(x,a,vis,adj,dhuka);
        }
        return;
}
