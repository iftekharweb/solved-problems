...
{
        vis[x][y] = 1;
        if(!ok) vs.push_back({x,y});
        else ve.push_back({x,y});
        for(int i=0; i<4; i++) {
                if(is_valid(x+dx[i], y+dy[i])) fun(x+dx[i], y+dy[i]);
        }
        return;
}
...
int main()
{
        ...
        for(int i=0; i<vs.size(); i++){
                for(int j=0; j<ve.size(); j++){
                        int p = (vs[i].first-ve[j].first)*(vs[i].first-ve[j].first);
                        int q = (vs[i].second-ve[j].second)*(vs[i].second-ve[j].second);
                        if(p+q < mi) mi = p+q;
                }
        }
        cout << mi << endl;
